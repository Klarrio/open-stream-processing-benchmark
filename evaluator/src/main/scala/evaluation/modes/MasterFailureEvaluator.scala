...
/**
  * Computes metrics that are used to analyze the master failure workload
  *
  ...
  * @param sparkSession
  */
class MasterFailureEvaluator (data: Dataset[MetricObservation], runTimes: DataFrame, evaluationConfig: EvaluationConfig, sparkSession: SparkSession) {
  ...
  def run(): Unit = {
    ...
    // Source: Others
    val latency = latencyUtils.computeForMasterFailure(data)
