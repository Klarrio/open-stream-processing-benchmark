...
class HdfsResourceComputer(runTimes: DataFrame, sparkSession: SparkSession, evaluationConfig: EvaluationConfig){

  import sparkSession.implicits._

  val runConfigColumns: Array[Column] = runTimes.select("runConfiguration.*").columns.map("runConfiguration." + _).map(col)

  def wrangleHdfsCadvisorMetrics() {
    val cadvisorMetrics = sparkSession.read.option("inferSchema", true).json(evaluationConfig.cadvisorHdfsMetricsFrameworkPath)
    ...
    cleanedCadvisorMetrics
      ...
      .option("header", "true")
      .csv(evaluationConfig.dataOutputPath("hdfs-cadvisor-metrics-per-container-timeseries"))
  }
