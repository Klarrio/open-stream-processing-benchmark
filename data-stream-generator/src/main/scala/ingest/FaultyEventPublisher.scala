 ...
 * Publishes an erroneous event after 10 minutes.
 *
 * @param sparkSession
 * @param kafkaProperties
 */

class FaultyEventPublisher(sparkSession: SparkSession, kafkaProperties: Properties) extends Publisher {
  ...
  override def publish(index: Int) = Future {
    ...
    val sendErrorTimestamp = nextSecond + (60 * 10.toInt * 1000) // Send the faulty event 10 minutes after the start
    ...
    fileOrderedDF.foreach { groupOfObservations: ((Long, Iterable[Observation])) => // SUPPOSED TO LAST ONE SECOND
      ...
      val sendError: Boolean = sendErrorTimestamp == nextSecond
