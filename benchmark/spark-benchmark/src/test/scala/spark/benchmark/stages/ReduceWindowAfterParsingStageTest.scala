 ...
 **/
class ReduceWindowAfterParsingStageTest extends FunSuite with StreamingSuiteBase {
  ...
  test("aggregate over sliding window per measurement ID") {
    ...
    testOperation(parsedFlowEvents, statefulStages.reduceWindowAfterParsingStage _, expectedResultOfAggregation, ordered = false)
  }
}
