...
{
   # Display Help
   echo "Kafka Streams worker failure workload"
   echo
   echo "Parameters:"
...
export MODE="worker-failure"; echo "- MODE = $MODE"
...
do
    ...
    dcos marathon app kill /kafka-streams/kafka-thread-1
...
for topic in  "${topicnames[@]}"; do
    echo "starting evaluator for $topic"
...
for i in  "${!topicnames[@]}"; do
    ...
    ./run-evaluator.sh $FRAMEWORK $MODE $LAST_STAGE "${topicnames[$i]}" ${begintimes[$i]} ${endtimes[$i]} $AMT_WORKERS $WORKER_CPU $WORKER_MEM
done
