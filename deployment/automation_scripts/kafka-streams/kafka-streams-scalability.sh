...
{
   # Display Help
   echo "Kafka Streams scalability workload"
   echo
   echo "The throughput levels that need to be tested need to be defined in the volume_array variable in the script."
   echo
   echo "Parameters:"
   echo "-l    LAST_STAGE: describing the last stage (REQUIRED)"
   echo "-a    AMT_WORKERS: number of workers in the cluster (REQUIRED)"
   echo "-c    WORKER_CPU: number of CPUs per worker in the cluster (REQUIRED)"
...
while getopts "hl:a:c:" option; do
   case $option in
      h) # display Help
          Help
          exit;;
      l)
          export LAST_STAGE=${OPTARG}; echo "- LAST_STAGE = $LAST_STAGE" ;;
      a)
          export AMT_WORKERS=${OPTARG}; echo "- AMT_WORKERS = $AMT_WORKERS" ;;
      c)
          export WORKER_CPU=${OPTARG}; echo "- WORKER_CPU = $WORKER_CPU" ;;
      \?)
...
volume_array=(0 100 200 300); echo "VOLUMES: ${volume_array[@]}"
...
# depending env vars
export WORKER_MEM=$(($WORKER_CPU*5)); echo "- WORKER_MEM = $WORKER_MEM"
export WORKER_MEM_MB=$(($WORKER_MEM*1024)); echo "- WORKER_MEM_MB = $WORKER_MEM_MB"
export WORKER_HEAP_MEM_MB=$(($WORKER_MEM_MB/2)); echo "- WORKER_HEAP_MEM_MB = $WORKER_HEAP_MEM_MB"
export NUM_PARTITIONS=$(($WORKER_CPU*$AMT_WORKERS)); echo "- NUM_PARTITIONS = $NUM_PARTITIONS"
export NUM_THREADS_PER_INSTANCE=$(($NUM_PARTITIONS/$AMT_WORKERS)); echo "- NUM_THREADS_PER_INSTANCE = $NUM_THREADS_PER_INSTANCE"
echo
echo
...
do
    ...
    ./create-kafka-topic.sh non-incremental-window-data-topic-$TOPICNAME $NUM_PARTITIONS
    ...
    echo "Starting up kafka streams job for stage $LAST_STAGE: buffer timeout $BUFFER_TIMEOUT and volume $DATA_VOLUME"
    ...
    do
        sleep 1m
        echo "benchmark running for $k minutes for buffer timeout $BUFFER_TIMEOUT and volume $DATA_VOLUME"
    done
