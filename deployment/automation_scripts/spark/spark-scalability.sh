...
{
   # Display Help
   echo "Spark Streaming scalability workload"
   echo
...
#hardcoded variables
export FRAMEWORK="SPARK"; echo "- FRAMEWORK = $FRAMEWORK"
export MODE="constant-rate"; echo "- MODE = $MODE"
export JAR_PATH=`cat ../benchmark-jars-path`
export JAR_NAME=$JAR_PATH/spark-benchmark-assembly-3.0.jar; echo "- JAR_NAME = $JAR_NAME"
...
export NUM_SQL_PARTITIONS=$NUM_PARTITIONS
export SPARK_CORES_MAX=$(($WORKER_CPU*$AMT_WORKERS))
...
do
  ...
  echo "starting volume $DATA_VOLUME"
  ################## RUN PREPARATION ######################
