...
{
   # Display Help
   echo "Flink master failure workload"
   echo
   echo "Parameters:"
...
cd ..
./start-flink-cluster-HA.sh $AMT_WORKERS $WORKER_CPU
...
do
  ...
  dcos marathon app kill flink-jobmanager
  ...
  dcos marathon app remove jmx-exporter
	...
	dcos marathon app stop flink-jobmanager-standby
  dcos marathon app remove flink-jobmanager-standby

	# kill streaming job
	echo "Killing streaming job for stage $LAST_STAGE"
  ssh core@$FLINK_JOBMANAGER_IP_ADDR docker exec -i $FLINK_JOBMANAGER_DOCKER_ID 'bash -c "./cancel-job.sh"'
